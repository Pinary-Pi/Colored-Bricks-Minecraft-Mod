...
public class ModRecipesProvider extends FabricRecipeProvider {
    ...
    protected RecipeGenerator getRecipeGenerator(RegistryWrapper.WrapperLookup registryLookup, RecipeExporter exporter) {
        return new RecipeGenerator(registryLookup, exporter) {
            private static final List<ItemConvertible> DYEABLES = List.of(
                Items.BRICK,
            ...
            private static final List<ItemConvertible> DYES = List.of(
                Items.WHITE_DYE,
                Items.ORANGE_DYE,
                Items.PINK_DYE,
                Items.YELLOW_DYE,
            ...
            private static final List<Pair<ItemConvertible, ItemConvertible>> COMPACT_2BY2_INPUTS_OUTPUTS = List.of(
                Pair.with(ModItems.WHITE_BRICK, ModBlocks.WHITE_BRICKS),
            ...
            private static final List<ItemConvertible> BLOCKS = List.of(
            ...
            public void offerCrackingRecipe(RecipeExporter exporter, ItemConvertible output, ItemConvertible input) {
            ...
            private void offerDyeableRecipes(RecipeExporter exporter, List<ItemConvertible> dyes, List<ItemConvertible> dyeables, RegistryEntryLookup<Item> itemLookup) {
            ...
            public void generate() {
                ...
                // Dye Recipes
                offerDyeableRecipes(exporter, DYES, DYEABLES, itemLookup);
                ...
                for (Pair<ItemConvertible, ItemConvertible> pair : COMPACT_2BY2_INPUTS_OUTPUTS) {
                    ...
                    offer2x2CompactingRecipe(RecipeCategory.MISC, output, input);
                }
                ...
                for (ItemConvertible block : BLOCKS) {
                    ...
                    offerSlabRecipe(RecipeCategory.MISC, slab, block);
                    createStairsRecipe(stair, Ingredient.ofItems(block))
                        .criterion(hasItem(block), conditionsFromItem(block))
                        .offerTo(exporter);
                    offerWallRecipe(RecipeCategory.MISC, wall, block);
                    if (Registries.ITEM.getId(block.asItem()).getPath().contains("cracked")) {
                        continue;
                    } else {
                        ItemConvertible chiseled = getChiseledVersion(block);

                        offerChiseledBlockRecipe(RecipeCategory.MISC, chiseled, slab);
                    }
                }

                offerChiseledBlockRecipe(RecipeCategory.MISC, ModBlocks.CHISELED_BRICKS, Blocks.BRICK_SLAB);
                ...
                for (Quintet<ItemConvertible, ItemConvertible, ItemConvertible, ItemConvertible, ItemConvertible> set : STONECUTTING_BLOCKS) {
                    ...
                    if (Registries.ITEM.getId(block.asItem()).getPath().contains("cracked")) {
                        offerStonecuttingRecipe(RecipeCategory.MISC, slab, block, 2);
                        offerStonecuttingRecipe(RecipeCategory.MISC, stair, block);
                        offerStonecuttingRecipe(RecipeCategory.MISC, wall, block);
                    } else {
                        offerStonecuttingRecipe(RecipeCategory.MISC, slab, block, 2);
                        offerStonecuttingRecipe(RecipeCategory.MISC, stair, block);
                        offerStonecuttingRecipe(RecipeCategory.MISC, wall, block);
                        offerStonecuttingRecipe(RecipeCategory.MISC, chiseled, block);
                    }
                }

                offerStonecuttingRecipe(RecipeCategory.MISC, ModBlocks.CHISELED_BRICKS, Blocks.BRICKS);
                ...
                for (Pair<ItemConvertible, ItemConvertible> pair : COMPACT_2BY2_INPUTS_OUTPUTS) {
                    ...
                    ShapedRecipeJsonBuilder.create(itemLookup, RecipeCategory.MISC, pot)
                        .input('#', brick)
                        .pattern("# #")
                        .pattern(" # ")
