...
public class ModRecipesProvider extends FabricRecipeProvider {
    ...
    public static void offerCrackingRecipe(Consumer<RecipeJsonProvider> exporter, ItemConvertible output, ItemConvertible input) {
		CookingRecipeJsonBuilder.createSmelting(Ingredient.ofItems(input), output, 0.1F, 200)
			.criterion(hasItem(input), conditionsFromItem(input))
			.offerTo(exporter, "cracking_" + Registry.ITEM.getId(input.asItem()).getPath());
	}
    ...
    private static void addStoneCuttingBlocks() {
        for (ItemConvertible block : BLOCKS) {
            if (Registry.ITEM.getId(block.asItem()).getPath().contains("cracked")) {
    ...
    private static Item getSlabVersion(ItemConvertible block) {
        String block_id = Registry.ITEM.getId(block.asItem()).getPath();
        String slab_id = block_id.replace("bricks", "brick_slab");
        return Registry.ITEM.get(new Identifier(ColoredBricks.MOD_ID, slab_id));
    }

    private static Item getStairVersion(ItemConvertible block) {
        String block_id = Registry.ITEM.getId(block.asItem()).getPath();
        String stair_id = block_id.replace("bricks", "brick_stairs");
        return Registry.ITEM.get(new Identifier(ColoredBricks.MOD_ID, stair_id));
    }

    private static Item getWallVersion(ItemConvertible block) {
        String block_id = Registry.ITEM.getId(block.asItem()).getPath();
        String wall_id = block_id.replace("bricks", "brick_wall");
        return Registry.ITEM.get(new Identifier(ColoredBricks.MOD_ID, wall_id));
    }

    private static Item getChiseledVersion(ItemConvertible block) {
        String block_id = Registry.ITEM.getId(block.asItem()).getPath();
        String chiseled_id = block_id.replace("bricks", "chiseled_bricks");
        ColoredBricks.LOGGER.info(Registry.ITEM.get(new Identifier(ColoredBricks.MOD_ID, chiseled_id)).toString());
        return Registry.ITEM.get(new Identifier(ColoredBricks.MOD_ID, chiseled_id));
    }
    ...
    private static Item getPotVersion(ItemConvertible block) {
        ...
        return Registry.ITEM.get(new Identifier(ColoredBricks.MOD_ID, pot_id));
    }

    private static void offerDyeableRecipes(Consumer<RecipeJsonProvider> exporter, List<ItemConvertible> dyes, List<ItemConvertible> dyeables) {
        for (ItemConvertible dyeable : dyeables) {
            for (ItemConvertible dye : dyes) {
                ...
                ShapelessRecipeJsonBuilder.create(output)
                    ...
                    .offerTo(exporter, "dye_" + Registry.ITEM.getId(output).getPath());
            }
        }
    }

    private static Item getDyedVersion(ItemConvertible dyeable, ItemConvertible dye) {
        String dye_id = Registry.ITEM.getId(dye.asItem()).getPath();
        String dyeable_id = Registry.ITEM.getId(dyeable.asItem()).getPath();
        String dyed_version_id = dye_id.replace("dye", "") + dyeable_id;
        ColoredBricks.LOGGER.info(Registry.ITEM.get(new Identifier(ColoredBricks.MOD_ID, dyed_version_id)).toString());
        return Registry.ITEM.get(new Identifier(ColoredBricks.MOD_ID, dyed_version_id));
    }
    ...
    public void generateRecipes(Consumer<RecipeJsonProvider> exporter) {
        ...
        for (Pair<ItemConvertible, ItemConvertible> pair : COMPACT_2BY2_INPUTS_OUTPUTS) {
            ...
            offer2x2CompactingRecipe(exporter, output, input);
        }
        ...
        for (ItemConvertible block : BLOCKS) {
            ...
            if (Registry.ITEM.getId(block.asItem()).getPath().contains("cracked")) {
                ...
                offerChiseledBlockRecipe(exporter, chiseled, slab);
            }
        }

        offerChiseledBlockRecipe(exporter, ModBlocks.CHISELED_BRICKS, Blocks.BRICK_SLAB);
        ...
        for (Quintet<ItemConvertible, ItemConvertible, ItemConvertible, ItemConvertible, ItemConvertible> set : STONECUTTING_BLOCKS) {
            ...
            if (Registry.ITEM.getId(block.asItem()).getPath().contains("cracked")) {
                offerStonecuttingRecipe(exporter, slab, block, 2);
                offerStonecuttingRecipe(exporter, stair, block);
                offerStonecuttingRecipe(exporter, wall, block);
            } else {
                offerStonecuttingRecipe(exporter, slab, block, 2);
                offerStonecuttingRecipe(exporter, stair, block);
                offerStonecuttingRecipe(exporter, wall, block);
                offerStonecuttingRecipe(exporter, chiseled, block);
            }
        }

        offerStonecuttingRecipe(exporter, ModBlocks.CHISELED_BRICKS, Blocks.BRICKS);
        ...
        for (Pair<ItemConvertible, ItemConvertible> pair : COMPACT_2BY2_INPUTS_OUTPUTS) {
            ...
            ShapedRecipeJsonBuilder.create(pot)
                .input('#', brick)
                .pattern("# #")
                .pattern(" # ")
    ...
    private static void offer2x2CompactingRecipe(Consumer<RecipeJsonProvider> exporter, ItemConvertible output, ItemConvertible input) {
